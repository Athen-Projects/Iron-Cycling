...
globals[
  ...
  new-oxygen-value ;;list of pairs [ticks, percentage] used to switch oxygen-saturation at time "ticks" to "percentage"
]
...
to setup
  ...
  set oxygen-csv csv:from-file "/home/natalie/netlogo_programs/Iron-Cycling-master/OxygenValues.csv"
end
...
to switch-oxygen
  ...
  [
    set new-oxygen-value item 0 oxygen-csv
    if (ticks = (item 0 new-oxygen-value))
    [
      set O2-saturation item 1 new-oxygen-value
...
to fe2-oxidation-by-air
  ask patches with [(fe? = True) and (redox-balance < 0)] [
    ;;minimum amount of iron oxidized. E. g. oxidized-iron-int = 4 for redox-balance * chemical-oxidation-factor = -4.5
    let oxidized-iron-int -1 * (ceiling (redox-balance * chemical-oxidation-factor))
